...
DitherJS.prototype.dither = function dither(target, options) {
    ...
    if (typeof target === 'string') {
        return this._fromSelector(target, options);
    }
    ...
    if (
        ...
        return this._fromImgElement(target, options);
    }

    if (
        typeof target === 'object' &&
        target instanceof window.HTMLCanvasElement
    ) {
        return this._fromCanvasElement(target, options);
    }

    console.log('ditherjs cannot process', target);
};
...
DitherJS.prototype._fromCanvasElement = function(el, options) {
